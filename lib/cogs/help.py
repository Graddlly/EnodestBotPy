...
def syntax(command):
	...
	params = [f"[{key}]" if "NoneType" in str(value) else f"<{key}>" for key, value in command.params.items() if key not in ("self", "ctx")]
...
class HelpMenu(ListPageSource):
	...
	async def write_page(self, menu, fields=[]):
		...
		embed = Embed(title = "Help", description = "Welcome to the EnodestCraftBot help dialog!", colour = self.ctx.author.colour)
