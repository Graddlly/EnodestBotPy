...
class MusicPlayer(Cog):
    ...
    async def filename_generator(self):
        ...
        name = ''.join(random.choice(chars) for _ in range(random.randint(9, 25)))
    ...
    async def queue(self, msg, song):
        ...
        return await msg.send(f"Added to queue", delete_after = 20)
    ...
    async def play(self, msg, *, song):
        if msg.guild.id in self.player:
            ...
            self.player[msg.guild.id] = {'player': None, 'queue': [], 'author': msg, 'name': None, "reset": False, 'repeat': False, 'volume': 0.5}
            return await self.start_song(msg, song)
    ...
    async def volume(self, msg, vol: int):
        vol = min(vol, 200)
        vol /= 100
        if (msg.author.voice is not None and msg.voice_client is not None and msg.voice_client.channel == msg.author.voice.channel and msg.voice_client.is_playing() is True):
